...
* readxl, lubridate, tidyverse 패키지를 불러온다. 
...
* Journal Entry data 객체를 Console에 확인  
...
* 전기 시산표에서도 결측을 없앤다.
* 결측을 없애고 나서 View(CYTB) 기능으로 읽어들인 시산표를 보니 99행이 시산표 중 재무상태표의 마지막 행이다. 시산표의 재무상태표 부분을 나누기 위하여 1행 부터 99행까지를 썰어낸다.(slice)
...
* 차이가 0보다 큰 항목 갯수 확인.
* A03 객체에서 차이가 0보다 큰 항목을 찾아 나타낸다. 이익잉여금 항목 하나만 나타나면 이상이 없다고 생각할 수 있다.
