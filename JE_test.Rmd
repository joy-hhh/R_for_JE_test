...
```{r}
## MUS sampling 함수 생성
mus_sampling <- \(SR, RC, PL, PM, EA, pop){
...
pop <- je_tbl |> 
  rename(amount = CR) |>    # mus_sampling 함수 실행을 위하여 금액 열 이름을 amount로 변경.
  filter(ACCTCD %in% acc) |> 
  mutate(JENO = as.character(JENO))

## 함수 실행
mus_sampling(SR, RC, PL, PM, EA, pop) # pop : 샘플링 대상 모집단
